...
const formatLocation = async (db, location) => {
  ...
  const subLocationsDetails = await Promise.all(
    formattedLocation.subLocations.map(
      subLocation => getSubLocationDetails(db, subLocation)
    )
  )
...
const formatLocationsList = async (db, locations) => {
  const locationsListPromises = locations.map(
    location => formatLocation(db, location)
  )
  const formattedLocationsList = await Promise.all(locationsListPromises)
  return formattedLocationsList
}

export default formatLocationsList
